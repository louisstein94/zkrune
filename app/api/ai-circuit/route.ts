...
export async function POST(req: NextRequest) {
  // A17: per-IP rate limit, same reasoning as /api/ai-chat. Circuit
  // generation is heavier, so cap is slightly lower (5/min).
  const { allowed, resetAt } = checkIpBucket('ai-circuit', req, {
    windowMs: 60_000,
    limit: 5,
  });
  if (!allowed) {
