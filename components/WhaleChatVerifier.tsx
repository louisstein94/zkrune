...
import type { WhaleTokenConfig } from '@/lib/whaleTokens';
...
// ── Snapshot cache (per snapshot URL) ────────────────────────────────────────
const snapshotCacheByUrl = new Map<string, import('@/lib/merkle').Snapshot>();

async function loadSnapshot(url: string): Promise<import('@/lib/merkle').Snapshot> {
  const cached = snapshotCacheByUrl.get(url);
  if (cached) return cached;
  const res = await fetch(url);
  if (!res.ok) throw new Error('Failed to load snapshot. Please try again later.');
  const snap = await res.json();
  snapshotCacheByUrl.set(url, snap);
  return snap;
}

// ── Main component ────────────────────────────────────────────────────────────
export default function WhaleChatVerifier({ config }: { config: WhaleTokenConfig }) {
  ...
  // ── Fetch Merkle path (client-side lookup from static snapshot) ──────────────
  const fetchPath = useCallback(async (address: string) => {
    setPhase('fetching');
    setErrorMsg('');

    try {
      const snapshot = await loadSnapshot(config.snapshotUrl);
      ...
      if (BigInt(entry.balance) < BigInt(config.threshold)) {
  ...
  }, [config.snapshotUrl, config.threshold]);
  ...
  const generateProof = async () => {
    ...
    try {
      ...
      addLine(`>                 minimumBalance=${config.threshold.toLocaleString('en-US')}`);
      ...
      const inputs = {
        ...
        root:            pathData.root,
        minimumBalance:  config.threshold.toString(),
      };
  ...
  const exportProof = () => {
    ...
    const exportData = {
      ...
      token: config.symbol,
      mint: config.mint,
      minimumBalance: config.threshold,
    ...
    a.href = url;
    a.download = `${config.key}-whale-proof.json`;
    a.click();
      ...
      <main className="pt-24 pb-20 px-4 sm:px-8 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="font-hatton text-5xl sm:text-6xl text-white mb-4">
            {config.name} <span className="text-zk-primary">Whale</span> Chat
          </h1>
          <p className="text-zk-gray text-lg max-w-2xl mx-auto leading-relaxed">
            Prove you hold{' '}
            <span className="text-white font-semibold">
              {config.thresholdPercent}%+ ({config.threshold.toLocaleString('en-US')} {config.symbol})
            </span>{' '}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-10">
          {[
            { n: '01', color: 'text-zk-primary', border: 'border-zk-primary/20', title: 'Snapshot', desc: `A Poseidon Merkle tree of all ${config.symbol} holders is taken at a specific block.` },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-zk-gray/50 text-xs font-mono">
                  threshold: {config.threshold.toLocaleString('en-US')} {config.symbol} · Poseidon Merkle · depth=20
                </p>
              </div>
            </div>
            {pathData && (
              <div className={`px-3 py-1.5 rounded-full text-xs font-mono border ${
                BigInt(pathData.balance) >= BigInt(config.threshold)
                  ? 'bg-zk-primary/10 border-zk-primary/30 text-zk-primary'
                  : 'bg-red-500/10 border-red-500/30 text-red-400'
              }`}>
                {BigInt(pathData.balance) >= BigInt(config.threshold) ? '🐋 Whale' : '✗ Below threshold'}
              </div>
            )}
          </div>
              ...
              <div className="text-center py-8 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-zk-gray/50 text-xs mt-3">
                    Contact the {config.name} team to request a snapshot refresh.
                  </p>
                </div>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-2xl border border-white/10 bg-black/20 p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                    {[
                      { label: 'Balance', value: `${Number(pathData.balance).toLocaleString('en-US')} ${config.symbol}`, highlight: BigInt(pathData.balance) >= BigInt(config.threshold) },
                  ...
                  <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${BigInt(pathData.balance) >= BigInt(config.threshold) ? 'bg-zk-primary' : 'bg-red-500/60'}`}
                      style={{ width: `${Math.min((Number(pathData.balance) / config.threshold) * 100, 100)}%` }}
                    />
                  </div>
                  <div className="flex justify-between text-xs text-zk-gray/40 mt-1 font-mono">
                    <span>0</span>
                    <span>Threshold: {config.threshold.toLocaleString('en-US')}</span>
                  </div>
                </div>
                ...
                {BigInt(pathData.balance) >= BigInt(config.threshold) ? (
                  ...
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5 text-center">
                    ...
                    <p className="text-zk-gray text-sm">
                      You need at least <span className="text-white">{config.threshold.toLocaleString('en-US')} {config.symbol}</span>.
                      You are <span className="text-white">{(config.threshold - Number(pathData.balance)).toLocaleString('en-US')}</span> tokens short.
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="relative rounded-2xl border border-zk-primary/40 bg-gradient-to-br from-zk-primary/10 to-transparent p-6 overflow-hidden">
                  ...
                  <div className="relative flex flex-col sm:flex-row items-center gap-5">
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <p className="text-zk-gray text-sm">
                        Merkle + ZK proof verified. {config.thresholdPercent}%+ holder confirmed — no identity disclosed.
                      </p>
                    </div>
