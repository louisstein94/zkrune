...
program.name("zkrune").description("CLI tool for zkRune - Compile and manage zero-knowledge circuits").version("1.0.0");
program.command("create <name>").description("Create a new circuit from template").option("-t, --template <template>", "Template to use (age-verification, balance-proof, etc.)").action(async (name, options) => {
  try {
...
program.command("compile <circuit>").description("Compile a Circom circuit").option("-o, --output <dir>", "Output directory", "./build").option("--optimize", "Optimize circuit constraints").action(async (circuit, options) => {
  try {
...
program.command("test <circuit>").description("Test a circuit with sample inputs").option("-i, --input <file>", "Input JSON file").action(async (circuit, options) => {
  try {
...
program.command("templates").description("List all available circuit templates").action(() => {
  listTemplates();
});
program.command("init").description("Initialize a new zkRune project").action(async () => {
  try {
    await initProject();
  } catch (error) {
    console.error(import_chalk6.default.red("Error:"), error.message);
